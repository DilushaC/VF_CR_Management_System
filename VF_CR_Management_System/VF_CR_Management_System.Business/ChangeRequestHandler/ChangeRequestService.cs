...
namespace VF_CR_Management_System.Business.ChangeRequestHandler
{
    public class ChangeRequestService : IChangeRequestService
    {
        ...
        {
            // 1. Fetch StepID dynamically from WorkflowStep for 'Department Head Approval'
            const string getAssessmentStepSql = @"
                SELECT TOP 1 StepID 
                FROM [CRManagementDB].[dbo].[WorkflowStep] 
            ...
            // 2. Fetch StatusID dynamically from CRStatus for 'Security'
            const string getStatusIdSql = @"
                SELECT TOP 1 StatusID 
                FROM [CRManagementDB].[dbo].[CRStatus] 
            ...
            int securityStatusId = Convert.ToInt32(statusIdObj);
